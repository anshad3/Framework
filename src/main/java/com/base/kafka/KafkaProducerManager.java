...
import com.base.reports.ExtentReportUtil;
...
public class KafkaProducerManager {
	...
	public static KafkaProducer<String, MessageContentPojo> kafkaProducer;

	
	public static void inistialiseProducer() {

		String kafkaServerIp = ReportLogger.caPropMap.get("KafkaServerIp");
		String kafkaServerPort = ReportLogger.caPropMap.get("KafkaServerPort");

		Properties properties = new Properties();
		properties.put("bootstrap.servers", kafkaServerIp + ":"+ kafkaServerPort);
		properties.put("key.serializer","org.apache.kafka.common.serialization.StringSerializer");
		properties.put("value.serializer","com.ca.base.kafka.ProducerPojoSerialiser");

		kafkaProducer = new KafkaProducer<String, MessageContentPojo>(properties);
	...
	public synchronized static void logInfo(Status testStatus,String stepName) {
		...
		writePojoObjectToProducer("logInfo",pojo);
	...
	public synchronized static void logScreenShot(Status testStatus,
		...
		writePojoObjectToProducer("logScreenShot",pojo);
	}
	...
	public synchronized static void closeReport() {
		...
		writePojoObjectToProducer("closeReport",null);
	...
	public synchronized static void initialiseReportForAtTest(ITestResult result) {
		...
		writePojoObjectToProducer("initialiseReportForAtTest",producerPojo);
	...
	public synchronized static void closeReportForAtTest(ITestResult testResult,String testCaseNameForKafka) {
		...
		try {
			...
			writePojoObjectToProducer("closeReportForAtTest",pojo);
		}
	...
	public synchronized static String logTestCaseIDandDesc(ITestResult result) {
		...
		writePojoObjectToProducer("logTestCaseName",pojo);
	...
	public synchronized static void displayServerLog(String testCaseID,
		...
		outputFile = "<a href=" + reldirFile + " target=\"_blank\" >" + logDisplayName + "</a>";
		 ...
		 writePojoObjectToProducer("displayServerLog",pojo);
	...
	private static void writePojoObjectToProducer(String operation,MessageContentPojo producerPojo) {
		
		String kafkaTopic = ReportLogger.caPropMap.get("KafkaTopic");
		try{
			kafkaProducer.send(new ProducerRecord<String, MessageContentPojo>(kafkaTopic,operation, producerPojo ));
